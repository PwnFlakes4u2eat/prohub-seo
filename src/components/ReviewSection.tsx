...
export default function ReviewSection({ reviews, serviceName, townName }: ReviewSectionProps) {
  ...
  // Auto-advance carousel every 5 seconds (only if we have enough reviews)
  useEffect(() => {
  ...
  // Calculate which reviews to show for flip animation
  const slots = hasReviews 
    ? Array.from({ length: Math.min(perPage, displayReviews.length) }, (_, i) => {
        ...
        {!hasReviews && (
          <div className="max-w-md mx-auto text-center py-12">
            ...
            <p className="text-muted-foreground">
              Be the first to leave a review! Customer reviews will appear here as {serviceName.toLowerCase()} jobs are completed in {townName}.
            </p>
          </div>
        )}

        {/* Reviews with stats and flip cards */}
        {hasReviews && (
          <>
            {/* Stats Banner */}
            <div className="flex flex-wrap items-center justify-center gap-6 mb-10">
              ...
              <div className="text-muted-foreground">
                <span className="text-foreground font-semibold">{reviews.length}</span> verified review{reviews.length !== 1 ? 's' : ''}
              </div>
            </div>
            
            {/* 3D Flip Cards (only with real reviews) */}
            ...
            {/* Dot indicators (only if more than one page) */}
